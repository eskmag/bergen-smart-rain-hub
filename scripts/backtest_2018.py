...
`backtest_2018()` is a pure function returning a summary dict — reused by the
`/api/validation` endpoint and the regression test in `tests/test_backtest.py`.
Run as a script for a human-readable printout:
